// This is Chrome Extension/Repro Now/download.js
...
function passToFFmpegv2(json, video, nameOfVideo) {
  ...
  fetch(video)
    ...
    .then((buf) => {
      var videoDownloadSuccess = new Promise(function (resolve, reject) {
        try {
      ...
      videoDownloadSuccess
        ...
        .catch(function () {
          document.querySelector("#error").innerHTML =
            "<b>Oops.. Something Went Wrong.. Trying manual download using ffmpeg</b>";
          chrome.tabs.create({
            url: "download_manual.html?localStorageId=" + params.localStorageId,
          });
        });
...
onload = function () {
  if (params.localStorageId) {
    ...
    chrome.storage.local.get(videoName, function (result) {
      passToFFmpegv2(
        result[videoName].json,
        result[videoName].video,
        result[videoName].friendlyName
      );
    });
  }
